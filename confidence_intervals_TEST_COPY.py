...
class Coverage():
	...
	def  __init__(self,
		...
		self.intervals = {"test_data": {   "percentile":		("-","-"),
										   "normal":			("-","-"),
										   "basic":				("-","-"),
										   "bias_corrected":	("-","-"),
										   "BC_a":				("-","-")}}
	...
	def return_intervals(self):
		return self.intervals["test_data"]
	...
	def coverage_orch(self):
		...
		if self.method == "all":
			...
			for key in interval_roster:
					...
					self.intervals["test_data"][key] = interval_roster[key]
		else:
			#General try-catch for specific interval
			try:
				self.intervals[type_key][key] = interval_roster[self.method](type_roster[type_key])
	...
	def CI_percentile(self):
		...
		sort_sample = sorted(self.sample)
		...
		if not isinstance(self.perc_bounds[0]*sample_length, int):
			b_l = (sort_sample[int(np.ceil(self.perc_bounds[0]*sample_length)) - 1] +
				  sort_sample[int(np.floor(self.perc_bounds[0]*sample_length)) - 1])/2
		else:
			b_l = sort_sample[self.perc_bounds[0]*len(self.sample) - 1]

		if not isinstance(self.perc_bounds[1]*len(self.sample), int):
			b_u = (sort_sample[int(np.ceil(self.perc_bounds[1]*sample_length)) - 1] +
				  sort_sample[int(np.floor(self.perc_bounds[1]*sample_length)) - 1])/2
		else:
			b_u = sort_sample[self.perc_bounds[1]*sample_length - 1]
	...
	def CI_basic(self):
		...
		#Get errors
		errors = sorted([(boot_val - self.ref) for boot_val in self.sample])
		...
		if not isinstance(self.perc_bounds[0]*sample_length, int):
			e_l = (errors[int(np.ceil(self.perc_bounds[0]*sample_length)) - 1] +
				  errors[int(np.floor(self.perc_bounds[0]*sample_length)) - 1])/2
		else:
			e_l = errors[self.perc_bounds[0]*sample_length - 1]

		if not isinstance(self.perc_bounds[1]*sample_length, int):
			e_u = (errors[int(np.ceil(self.perc_bounds[1]*sample_length)) - 1] +
				  errors[int(np.floor(self.perc_bounds[1]*sample_length)) - 1])/2
		else:
			e_u = errors[self.perc_bounds[1]*sample_length - 1]

		#Construction of basic confidence interval
		conf_interval = (self.ref - e_u, self.ref - e_l)

		return conf_interval

	def CI_bias_corrected(self):
		"""
		...
		if not isinstance(Q_l*sample_length, int):
			b_l = (sort_sample[int(min(max(np.ceil(Q_l),1),sample_length) - 1)] +
				  sort_sample[int(min(max(np.floor(Q_l),1),sample_length) - 1)])/2

		else:
			b_l = sort_sample[min(max(1, Q_l), sample_length) - 1]

		if not isinstance(Q_u*sample_length, int):
			b_u = (sort_sample[int(max(min(sample_length, np.ceil(Q_u)), 1)) - 1] +
				  sort_sample[int(max(min(sample_length, np.floor(Q_u)), 1)) - 1])/2
		else:
			b_u = sort_sample[max(min(sample_length, Q_u), 1) - 1]
	...
	def CI_BC_a(self):
		...
		# If not, take the average to the two proximate values
		# Must subtract one because of zero indexing
		if not isinstance(adj_low_bound*sample_length, int):
			b_l = (sort_sample[int(min(max(np.ceil(adj_low_bound*sample_length),1),
										   sample_length) - 1)] +
				  sort_sample[int(min(max(np.floor(adj_low_bound*sample_length),1),
				  						   sample_length) - 1)])/2

		else:
			b_l = sort_sample[min(max(1, adj_low_bound*sample_length), 
									  sample_length) - 1]

		if not isinstance(adj_upper_bound*sample_length, int):
			b_u = (sort_sample[int(max(min(np.ceil(adj_upper_bound*sample_length), 
										  sample_length), 1)) - 1] +
				  sort_sample[int(max(min(np.floor(adj_upper_bound*sample_length),
				  						   sample_length), 1)) - 1])/2
		else:
			b_u = sort_sample[max(min(adj_upper_bound*sample_length, 
									  sample_length), 1) - 1]
	...
	def __z_0(self, sample):
		...
		numerator = np.size([item for item in sample if item < np.mean(sample)])
		...
		return norm.ppf(min(max(1,numerator),sample_length - 1)/
						float(sample_length))
	...
	def __theta_dot(self, sample):
		...
		theta_dot = theta_sum / sample_length
	...
	def __s_x_BC_a(self, sample):
		...
		s_x_i = (sq_dif_sum / sample_length)
